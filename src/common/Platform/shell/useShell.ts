...
// Tauri also runs on WebView2, so chrome.webview exists there without a Stremio
// shell behind it — the Tauri app talks to its Rust backend over its own IPC.
const isTauri = !!(globalThis as any).__TAURI_INTERNALS__;
const IPC = isTauri ? undefined : globalThis?.chrome?.webview;
const LEGACY_IPC = isTauri ? undefined : globalThis?.qt?.webChannelTransport;
